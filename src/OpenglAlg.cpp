...
void Mesh_view_data_init(Mesh_view_data *m)
{
   ...
   // m->F_V=NULL;
    ...
    int size=node_size(m->F_F),size1=node_size(m->E_V);
    
//printf("dsfds\n");
        m->F_color_size=(int*)malloc(sizeof(int)*size);
        m->F_color=(GLfloat**)malloc(sizeof(GLfloat*)*size);
    
    m->VAOs=(GLuint*)malloc(sizeof(GLuint));

    glGenVertexArrays(size+size1+5,m->VAOs);
    if(m->Buffers!=NULL)
    {
        free(m->Buffers);
    }
    m->Buffers=(GLuint**)malloc(sizeof(GLuint*)*(size+size1+1));  
